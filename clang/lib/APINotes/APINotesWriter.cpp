...
void APINotesWriter::addObjCMethod(ContextID contextID,
  ...
  if (info.DesignatedInit) {
    assert(Implementation->ObjCContexts.count(
        {Implementation->ObjCContextNames[contextID.Value], (char)0}));
    auto &versionedVec =
        Implementation
            ->ObjCContexts[{Implementation->ObjCContextNames[contextID.Value],
                            (char)0}]
            .second;
    bool found = false;
    for (auto &versioned : versionedVec) {
...
void APINotesWriter::addGlobalVariable(llvm::StringRef name,
  ...
  Implementation->GlobalVariables[variableID].push_back({swiftVersion, info});
}
...
void APINotesWriter::addGlobalFunction(llvm::StringRef name,
                                       ...
                                       VersionTuple swiftVersion) {
  IdentifierID nameID = Implementation->getIdentifier(name);
  Implementation->GlobalFunctions[nameID].push_back({swiftVersion, info});
}
...
void APINotesWriter::addTag(llvm::StringRef name, const TagInfo &info,
  ...
  Implementation->Tags[tagID].push_back({swiftVersion, info});
}
...
void APINotesWriter::addTypedef(llvm::StringRef name, const TypedefInfo &info,
  ...
  Implementation->Typedefs[typedefID].push_back({swiftVersion, info});
}
